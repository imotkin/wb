...
### 2. Метрики добавления и получения для кэша и базы данных, пинга для Kafka и PostgreSQL

Статус - **`Выполнено`**

Были добавлены дополнительные метрики для Prometheus:

- `cache_get_total` - общее число полученных заказов из кэша
- `cache_set_total` - общее число добавленных заказов в кэш
- `postgres_get_total` - общее число полученных заказов из базы данных
- `postgres_set_total` - общее число добавленных заказов в базу данных
- `kafka_status` - текущий статус (ping) для Kafka (1 - доступна, 0 - нет)
- `postgres_status` - текущий статус (ping) для PostgreSQL (1 - доступна, 0 - нет)

### 3. Добавить error wrapping

Статус - **`Выполнено`**

Добавил error wrapping для результатов функций и методов.

### 4. Тесты для базы данных

Статус - **`Выполнено`**

Был добавлен интеграционный тест для базы данных, включающий в себя:

- добавление заказа
- получение заказа
- добавление списка из 10 заказов
...
Для логгера были добавлены юнит-тесты.

### 6. Параметры для тайм-аута сервера в файле конфигурации

Статус - **`Выполнено`**

Для тайм-аута HTTP-сервера были добавлены параметры в файле конфигурации:
...
Был реализован один интеграционный тест - для базы данных.
...
Также был добавлен endpoint для получения списка всех заказов – [`http://localhost:8080/orders`](http://localhost:8080/orders)
